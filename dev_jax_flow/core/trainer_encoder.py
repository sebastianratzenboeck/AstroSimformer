...
class EMABuffer:
    ...
    def update(self, new):
        if self.buffer is None:
            ...
            self.buffer = self.decay * self.buffer + (1 - self.decay) * new
        return self.buffer


def mlp_encoder(hidden_dim=64, output_dim=32):
    def net(x):
        mlp = hk.Sequential([
...
class TrainFlowModel:
    def __init__(
        ...
        encode_mask=None,  # (D,) bool, indicating which features to encode
        batch_size=128,
        lr=1e-3,
        ...
        self.encoder = hk.without_apply_rng(hk.transform(mlp_encoder()))
        self.encoder_params = self.encoder.init(key, jnp.zeros((1, jnp.sum(self.encode_mask))))
    ...
    def encode_batch(self, batch):
        x_encoded = batch[:, self.encode_mask]
        x_pass = batch[:, ~self.encode_mask]
        z = self.encoder.apply(self.encoder_params, x_encoded)
        z_ema = self.ema_buffer.update(z)
        return jnp.concatenate([z_ema, x_pass], axis=-1)

    def loss_fn(self, params, key):
        batch_xs = self.random_batch(key)
        batch_xs = self.encode_batch(batch_xs)
