...
from train_interface.utils import save_params, load_params, file_with_largest_integer
...
class TrainScoreModel:
    ...
    def fit(self, epochs: int=1_000):
        ...
        replicated_params = jax.tree_map(lambda x: jnp.array([x] * n_devices), self.params)
        replicated_opt_state = jax.tree_map(lambda x: jnp.array([x] * n_devices), self.opt_state)
        ...
        self.opt_params = jax.tree_map(lambda x: x[0], best_params)
        return